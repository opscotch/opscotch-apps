...
## 1.10
- Lengthen poll period to 60s and GitHub host timeout to 30s to reduce timer skips under slow API responses

## 1.9
- Expand group-derived assignees into distinct GitHub polling groups and deduplicate direct/group logins.

## 1.8
- Do not touch host class Double when emitting poll metrics

## 1.7
- Send poll and error metrics with the timestamp sendMetric overload so a metric emit cannot stop the processor

## 1.6
- Emit GitHub poll, handoff, and HTTP error metrics

## 1.5
- Emit GitHub poll and handoff activity metrics via community `3.1.8`
...
- Release: https://github.com/opscotch/opscotch-apps/releases/tag/opscotch-github-issue-watcher-1.10
- Docker Image: `ghcr.io/opscotch/opscotch-github-issue-watcher:1.10`
- Catalog: https://github.com/opscotch/opscotch-apps/blob/main/apps/opscotch/opscotch-github-issue-watcher/1.10/README.md

To use this as a docker image, here is an example Dockerfile:

```Dockerfile
FROM ghcr.io/opscotch/opscotch-github-issue-watcher:1.10 AS opscotch-github-issue-watcher
