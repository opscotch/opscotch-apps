...
- Align workflow integration staging with hopscotch `TestFileResolver` semantics instead of MCP-specific path reconstruction.
- Stage config, bootstrap, manifest include, and mock response file references using the same root containment rules as standalone `opscotch-testrunner`.
- Dereference symlinks only for explicitly staged reference files; restore default directory staging behavior.

## 3.1.8.rc9

- Bump embedded testrunner and agent images to `3.1.8-10` / `3.1.8-10-dev`, picking up canonical test-file resolution from hopscotch 3.1.8.
...
- Docker Image: `ghcr.io/opscotch/opscotch-local-development-mcp:3.1.8.rc10`
- Catalog: https://github.com/opscotch/opscotch-apps/blob/main/apps/opscotch/opscotch-local-development-mcp/3.1.8.rc10/README.md